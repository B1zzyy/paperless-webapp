...
function decodeBase64UrlToJson(payload) {
...
function mapPayloadToReceiptData(rawPayload) {
  if (!rawPayload || typeof rawPayload !== 'object') {
    return null;
  }

  const splitPercent = safeNumber(rawPayload.splitPercent, 100);
  if (splitPercent < 1 || splitPercent > 100) {
    return null;
  }

  const sourceReceipt = rawPayload.receipt;
  ...
  return {
    ...
    receipt_snapshot: {
      store_name: sourceReceipt.storeName || '',
      purchase_date: sourceReceipt.purchaseDate || null,
...
export default function SharePayloadReceiptView() {
  ...
  if (payload) {
    try {
      const decoded = decodeBase64UrlToJson(payload);
      shared = mapPayloadToReceiptData(decoded);
    } catch (_err) {
      shared = null;
    }
  }
